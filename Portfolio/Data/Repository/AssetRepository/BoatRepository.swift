...
class BoatRepository: AssetRepositoryProtocol {
    ...
    func saveModel(with model: AssetDomainModel) async throws(NetworkError) {
        ...
        if let boatModel = model.assetDetails.model?.data(using: .utf8) {
            multipartData.append(boatModel, withName: "model")
        }

        if let HIN = model.assetDetails.HIN?.data(using: .utf8) {
            multipartData.append(HIN, withName: "HIN")
        }

        if let length = model.assetDetails.length, let lengthData = "\(length)".data(using: .utf8) {
            multipartData.append(lengthData, withName: "length")
        }
        ...
        do {
            let result: NoContentDto = try await networkManager.multipartRequest(
                endPoint: AssetEndpoints.saveBoatAsset,
